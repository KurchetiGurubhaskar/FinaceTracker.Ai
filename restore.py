...
pages = ['AiCenter', 'Analytics', 'BankConnect', 'Calendar', 'Dashboard', 'Finance', 'Lms', 'Reports', 'auth/ForgotPassword', 'auth/Login', 'auth/Register']
for p in pages:
    path = os.path.join(d, f'{p}.tsx')
    name = p.split('/')[-1]
    content = f'''import React from 'react';\n\nexport function {name}() {{\n  return (\n    <div className="p-8">\n      <h1 className="text-2xl font-bold">{name}</h1>\n      <p>This page was temporarily restored from a skeleton.</p>\n    </div>\n  );\n}}\n'''
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)

content_upload = """import React, { useState, useCallback, useEffect } from 'react';
...
"""
with open(os.path.join(d, 'UploadStatement.tsx'), 'w', encoding='utf-8') as f:
    f.write(content_upload)
